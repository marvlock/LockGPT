...
type ReviewChat = { id: string; title?: string; cleanupStatus: string };
function App() {
  ...
  const reviewUi = chats.length > 0 && <section className="review"><h2>Guest chats</h2><p>Choose which chats to keep in this account.</p>{chats.map((chat, i) => <label key={chat.id} className="chat-row"><input type="checkbox" checked={selected.includes(chat.id)} disabled={chat.cleanupStatus === 'deleted'} onChange={e => setSelected(e.target.checked ? [...selected, chat.id] : selected.filter(id => id !== chat.id))} /><span className="chat-title">{chat.title || `Guest chat ${i + 1}`}</span><small>{chat.cleanupStatus}</small></label>)}<button className="secondary" disabled={!selected.length} onClick={() => review('KEEP_GUEST_CHATS')}>Keep selected</button><button className="danger" disabled={!selected.length} onClick={() => review('DELETE_GUEST_CHATS')}>Delete selected</button></section>;
  return <main><header><span className="brand-mark"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={iconPaths.lock} /></svg></span><div><h1>LockGPT</h1><p className="eyebrow">Guest access for ChatGPT</p></div></header>
    {state.phase === 'UNCONFIGURED' ? <><h2>Set up guest access</h2><p>Create a PIN to keep your existing chats private when someone borrows your browser.</p><label>New PIN<input aria-label="New PIN" inputMode="numeric" type="password" value={pin} onChange={e => setPin(e.target.value)} /></label><label>Confirm PIN<input aria-label="Confirm PIN" inputMode="numeric" type="password" value={confirm} onChange={e => setConfirm(e.target.value)} /></label><small>Use at least 6 digits.</small><button onClick={() => submit('SET_PIN')}>Create PIN</button></> : (state.phase === 'LOCKED' || state.phase === 'LOCKING') ? <><h2>Guest session is on</h2><p>Your existing chats are hidden. Enter your PIN to return to your account.</p><label>PIN<input aria-label="PIN" autoFocus inputMode="numeric" type="password" value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit('UNLOCK')} /></label><button onClick={() => submit('UNLOCK')}>Unlock</button></> : <><h2>Share a fresh chat</h2><p>Hide your existing conversations across all ChatGPT tabs while a guest uses your browser.</p><button onClick={lock}>Lock &amp; start guest session</button>{state.lastGuestSessionId && <><button className="secondary" onClick={loadChats}>Review last guest session</button>{reviewUi}</>}</>}
